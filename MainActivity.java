...
public class MainActivity extends PreferenceActivity implements Preference.OnPreferenceChangeListener, Preference.OnPreferenceClickListener {
    ...
    public static String _tag = "demo_menu";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //check that either my application's service is Enabled or Not in "Notification Access"
        //Look for name in the database 中央資料庫
...
//重複使用 pPref 變數
    ...
    public void save(String filename,String data){
        ...
        try{
            ...
            //If not exist, the file will be created
    ...
    public boolean onPreferenceClick(Preference preference) {
        if (preference.getKey().equals("sdk_init_reinit")) {
            ...
            if (_goFITSdk != null) {
                ...
                new httpConnectTask().execute(mMacAddress);
            ...
            if (_goFITSdk != null) {
                ...
                _goFITSdk.doSyncFitnessData(
                        new GoFITSdk.SyncCallback() {
                    @Override
                    public void onCompletion() {
                        Log.i(_tag, "doSyncFitnessData() : onCompletion()");
                        showToast("Sync complete!\nDetail fitness data show in `Logcat`");

                        //NotificationListenerService.doSendIncomingEventToDevice(AppContract.emIncomingMessageType.Default,"Have you excercised ? ", "");
                    ...
                    public void onGetFitnessData(ArrayList<TableStepRecord> stepRecords, ArrayList<TableSleepRecord> sleepRecords, ArrayList<TablePulseRecord> hrRecords, ArrayList<TableSpO2Record> spo2Records) {
                        ...
                        sb.append("]");
                        save("stepTable.json",sb.toString());


                        StringBuilder sbForSleep = new StringBuilder();
                        ...
                        for (TableSleepRecord sleep : sleepRecords) {
                            ...
                            sbForSleep.append(sleep.toJSONString()+",");
                        }

                        if(sleepRecords.size()!=0){
                        sbForSleep.setLength(sbForSleep.length()-1);


                        }

                        Log.d("string",sbForSleep.toString());
                        //sbForSleep.delete(sb.lastIndexOf(","),sb.lastIndexOf(","));
                        sbForSleep.append("]");
                        save("sleepTable.json",sbForSleep.toString());

                        StringBuilder sbForPulse = new StringBuilder();
                        sbForPulse.append("[");
                        ...
                        for (TablePulseRecord hr : hrRecords) {
                            ...
                            sbForPulse.append(hr.toJSONString()+",");


                        }
                        if(hrRecords.size()!=0){

                        sbForPulse.setLength(sbForPulse.length()-1);
                        }
                        Log.d("string",sbForPulse.toString());
                        //sbForPulse.delete(sb.lastIndexOf(","),sb.lastIndexOf(","));

                        sbForPulse.append("]");
                        save("pulseTable.json",sbForPulse.toString());

                        StringBuilder sbForFitness = new StringBuilder();
                        sbForFitness.append("[");
                        for (TableSpO2Record spo2 : spo2Records) {
                            Log.i(_tag, "doSyncFitnessData() : onGetFitnessData() : spo2 = " + spo2.toJSONString());

                            sbForFitness.append(spo2.toJSONString()+",");
                        }
                        if(spo2Records.size()!=0){
                        sbForFitness.setLength(sbForFitness.length()-1);

                        }
                        Log.d("string",sbForFitness.toString());
                        //sb.delete(sb.lastIndexOf(","),sb.lastIndexOf(","));
                        sbForFitness.append("]");
                        save("fitnessTable.json",sbForFitness.toString());
